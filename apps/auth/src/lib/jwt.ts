...
const signToken = async (payload: JwtPayload, secret: string, type: TokenType, ttlSeconds: number): Promise<string> => {
	return jwt.sign({ ...payload, type, exp: Math.floor(Date.now() / 1000) + ttlSeconds }, secret, "HS256")
}

const verifyToken = async (token: string, secret: string, type: TokenType): Promise<JwtPayload | null> => {
	try {
...
const signAccessToken = (payload: JwtPayload, secret: string): Promise<string> => {
	return signToken(payload, secret, "access", ACCESS_TOKEN_TTL_SECONDS)
}

const signRefreshToken = (payload: JwtPayload, secret: string): Promise<string> => {
	return signToken(payload, secret, "refresh", REFRESH_TOKEN_TTL_SECONDS)
}

const verifyAccessToken = (token: string, secret: string): Promise<JwtPayload | null> => {
	return verifyToken(token, secret, "access")
}

const verifyRefreshToken = (token: string, secret: string): Promise<JwtPayload | null> => {
