...
1. Create a database on your TiDB cluster with `test` in its name — e.g. `CREATE DATABASE free_pos_test;`. The name is not cosmetic: `test/setup.ts` refuses to run against anything else, because teardown issues `DELETE`s.
...
- Tokens are delivered as `httpOnly`, `Secure`, `SameSite=Lax` cookies — `free_pos_access_token` (`Path=/`, optionally `Domain=.yourdomain.com` in prod so any `*.yourdomain.com` service receives it) and `free_pos_refresh_token` (`Path=/refresh`, always host-only). Never returned in a JSON response body. `login.handlers.ts`/`register.handlers.ts` set both via `setAuthCookies` (`@repo/auth-kit/cookies`) after creating the session; `refresh.handlers.ts` sets fresh ones on a successful rotation and clears both (`clearAuthCookies`) on any failure path, so a dead refresh token doesn't linger in the browser; `logout.handlers.ts` clears both unconditionally.
- CORS (`hono/cors`, mounted in `index.ts`) exact-matches `Origin` against `FREE_POS_FRONTEND_ORIGIN` and sets `credentials: true` — required for the browser to send/receive cookies cross-origin from `app.yourdomain.com`. No separate CSRF token: `SameSite=Lax` is the sole defense, which works because `app.yourdomain.com` and `auth.yourdomain.com` share the registrable domain `yourdomain.com` and so are same-site.
...
- **Shared HS256 secret**: widening the access-token cookie's `Domain` to `.yourdomain.com` means every future verifying service needs `FREE_POS_JWT_SECRET` distributed to it. Deferred until a second service actually exists (would move to RS256/JWKS).

## Conventions specific to this app

- Routes are mounted at the root in `index.ts` (`.route("/", xRoutes)`) — the service is deployed on its own `auth.yourdomain.com` subdomain, so an individual `*.routes.ts` file defines paths directly (e.g. `/register/email`), with no additional prefix.
- Handlers return **only** `c.json(...)` and use the pure builders from `@repo/hono-utils/response`: `successResponse(message, data)` for JSON successes and `errorResponse(payload)` for JSON errors. Keep OAuth redirects outside the envelope contract.
