...
`@repo/frontend`: client-side SPA built with Vite + React 19 + TanStack Router, no server component (no TanStack Start, no SSR). Renders entirely in the browser off a static bundle, deployed as a Cloudflare Workers static-assets Worker (`free-pos-frontend`), see Deploying below. Calls `apps/auth` via a typed Hono RPC client — see API client below.
...
Deployed as a static-assets-only Cloudflare Worker (`free-pos-frontend`, `wrangler.jsonc`) — no `main`/Worker script, since this app has no server code: `assets.directory` points at the Vite `dist/` build, and `assets.not_found_handling: "single-page-application"` serves `index.html` for unmatched routes so client-side routing works. Deployed from the `production` branch via a `deploy-frontend` job in `.github/workflows/deploy.yml`, parallel to (not dependent on) `apps/auth`'s deploy job — the two are independent artifacts with nothing to coordinate (no shared migration step). The production custom domain (`app.yourdomain.com`) is attached to the Worker manually via the Cloudflare dashboard rather than declared in `wrangler.jsonc`, mirroring how `apps/auth`'s `auth.yourdomain.com` is attached — see that app's `CLAUDE.md`. No PR preview deployments yet.
...
- `src/modules/auth/` holds the auth features: `components/` (`LoginForm.tsx`, `RegisterForm.tsx`, `ForgotPasswordForm.tsx`, `ResetPasswordForm.tsx`, `GoogleIcon.tsx`), `hooks/` (`useLoginForm.ts`, `useRegisterForm.ts`, `useForgotPasswordForm.ts`, `useResetPasswordForm.ts` — the TanStack Form instances, wired to `authApi` for the submit), `schemas/` (`login.schema.ts`, `register.schema.ts`, `forgot-password.schema.ts`, `reset-password.schema.ts` — valibot, mirroring the `apps/auth` schemas), `lib/api.ts` (the RPC client above). Follow this `modules/<feature>/` shape for future features rather than one flat `components/` directory.
